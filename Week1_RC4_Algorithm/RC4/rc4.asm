...
start:
    ...
    push offset bytesWritten        ; lpNumberOfCharsWritten - luon la gia tri dau ra, sau khi goi ham se tra ve tai bien nay
    push [szInputLen]               ; nNumberOfCharsToWrite - luon la gia tri dau vao, can phai khoi tao ngay khi khai bao
    push offset szInputPrompt       ; lpBuffer - luon la gia tri dau vao, duoc khai bao va khoi tao tai section .data
    push [hStdOut]                  ; hConsoleOutput - luon la gia tri dau vao, handle toi stdout
    call WriteConsoleA              ; Ham WriteConsoleA nhan vao 5 tham so WriteConsoleA(hConsoleOutput, *lpBuffer, nNumberOfCharsToWrite, lpNumberOfCharsWritten, lpReserved)
                                    ; khi goi ham WriteConsoleA, chuong trinh se lay lan luot tu dinh stack (bat dau tu phan tu duoc push gan nhat)
    
    ; Doc input tu ban phim
    push NULL                       ; pInputControl - luon la gia tri dau vao, khong bat buoc nhan gia tri cu the
    push offset bytesRead           ; lpNumberOfCharsRead - luon la gia tri dau ra, lay dia chi cua byte NULL sau khi nhan input
    push 255                        ; nNumberOfCharsToRead - luon la gia tri dau vao, la so ky tu toi da duoc phep doc
    push offset plaintext           ; lpBuffer - luon la gia tri dau ra, bien nay duoc gan sau khi da nhan input tu nguoi dung
    push [hStdIn]                   ; hConsoleInput - luon la gia tri dau vao, handle toi stdin
    call ReadConsoleA               ; Ham ReadConsoleA nhan vao 5 tham so ReadConsoleA(hConsoleInput, lpBuffer, nNumberOfCharsToRead, lpNumberOfCharsRead, pInputControl)
    ...
    push eax            ; eax dang luu gia tri length cua hexOutput, push vao stack de khong bi ghi de sau khi goi WriteConsoleA
